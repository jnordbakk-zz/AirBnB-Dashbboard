...
function tenRandomNumbers(ceiling){
    var arr = []
    while(arr.length < 10){
...
function getDashData() {
    d3.json(url, function(response) {
        ...
        myData.forEach(record => {
            var recData = {};
            recData = {
                State : record.property_type,
                freq:{
                    low:record.min,
                    mid:parseInt(record.mean),
                    high:record.max
                }
            };
            finalData.push(recData);
        });
        //console.log(finalData);
        var dashData = [];
        var randomSelection = tenRandomNumbers(finalData.length);
        randomSelection.forEach(number=>{
            dashData.push(finalData[number]);
        });
        // console.log(randomSelection);
        // console.log(dude);
        dashboard('#dashboard',dashData);
    });
}                    
getDashData();
